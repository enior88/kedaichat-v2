        ...
        <div className="min-h-screen bg-[#F8F9FA] pb-32 font-inter">
            ...
            <div className="bg-white px-6 pt-6 shadow-sm">
                ...
                {(storeInfo?.plan === 'FREE' && !storeInfo.isAdmin) && (
                    <div className="mb-6 bg-red-50 border border-red-100 p-6 rounded-[24px] text-center">
                        <h3 className="text-red-600 font-bold mb-2">Upgrade Required</h3>
                        <p className="text-red-500 text-sm mb-4">Reseller & Group Order networks are premium features.</p>
                        <button className="bg-red-600 text-white px-6 py-2 rounded-full text-sm font-bold shadow-lg shadow-red-200">
                            Upgrade to Basic
                        </button>
                    </div>
                )}

                <div className={`flex bg-gray-50 p-1.5 rounded-[20px] mb-4 ${(storeInfo?.plan === 'FREE' && !storeInfo.isAdmin) ? 'opacity-50 pointer-events-none' : ''}`}>
                    {tabs.map((tab) => (
                        <button
                            key={tab}
