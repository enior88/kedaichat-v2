...
export async function middleware(request: NextRequest) {
    ...
    const reservedPaths = ['group', 'admin', 'api', 'billing', 'dashboard', 'login', 'onboarding', 'orders', 'products', 'reseller', 'wallet', 'tools', 'shop', 'analytics', 'settings', 'checkout', 'privacy', 'terms', '_next', 'favicon.ico', 'logo.png', 'favicon.png', 'hero_illustration.png', 'robots.txt', 'sitemap.xml', 'ads.txt', 'kuih-gula-melaka.jpg', 'motorcycle-coffee.jpg', ...pwaAssets];
...
export const config = {
    matcher: ['/((?!api|_next/static|_next/image|favicon.ico|.*\\.jpg|.*\\.jpeg|.*\\.png|.*\\.webp|.*\\.svg|.*\\.gif|.*\\.js|.*\\.webmanifest|.*\\.json|.*\\.txt).*)'],
};
